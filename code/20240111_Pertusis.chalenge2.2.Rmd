...
yDF <- fcm2020DF %>% 
  ...
  select(subject_id, planned_day_relative_to_boost, percent_live_cell) %>%
  pivot_wider(names_from = planned_day_relative_to_boost, values_from = percent_live_cell, names_prefix = "perc_") %>%
...
```{r agua-fcm, eval=FALSE}
set.seed(seed= 3)
trainDF <- xDF %>%
  as.data.frame() %>%
  mutate(perc_FC = yDF$perc_FC)
...
x2DF <- scale(x2DF)
fit <- cv.glmnet(x = as.matrix(xDF), 
                 y = yDF$perc_FC[match(rownames(xDF), table = yDF$subject_id)],
                 nfolds = nrow(xDF),
                 grouped = FALSE)
plot(fit)
...
plot(yhat[, "s1"], y2DF$perc_FC)
cor.test(yhat[, "s1"], y2DF$perc_FC)

plot(rank(-1 *yhat[, "s1"]), rank(-1 * y2DF$perc_FC))
cor.test(rank(-1* yhat[, "s1"]), rank(-1* y2DF$perc_FC))
```
...
trainDF <- xDF %>%
  as.data.frame() %>%
  mutate(perc_FC = yDF$perc_FC[match(rownames(xDF), table = yDF$subject_id)])
...
plot(predict(auto_fit, new_data = as.data.frame(xDF))$.pred, 
     yDF$perc_FC[match(rownames(xDF), table = yDF$subject_id)])
...
plot(yhat[as.character(y2DF$subject_id), "s1"], y2DF$perc_FC)
cor.test(yhat[as.character(y2DF$subject_id), "s1"], y2DF$perc_FC)

plot(rank(-1 * yhat[as.character(y2DF$subject_id), "s1"]), rank(-1 * y2DF$perc_FC))
cor.test(rank(-1 * yhat[as.character(y2DF$subject_id), "s1"]), rank(-1 * y2DF$perc_FC))
```
