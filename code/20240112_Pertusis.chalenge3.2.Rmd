...
yDF <- ge2020DF %>% 
  ...
  filter(planned_day_relative_to_boost %in% c(0, 3)) %>%
  select(subject_id, planned_day_relative_to_boost, tpm) %>%
  pivot_wider(names_from = planned_day_relative_to_boost, values_from = tpm, names_prefix = "tpm_") %>%
  mutate(tpm_FC = tpm_3/tpm_0,
         tpm_FC = as.numeric(scale(tpm_FC)))
y2DF <- ge2021DF %>% 
  filter(versioned_ensembl_gene_id %in% "ENSG00000277632.1") %>%
...
plot(rank(-1 * yhat), rank(-1 * y2DF$tpm_FC))
cor.test(rank(-1 * yhat), rank(-1 * y2DF$tpm_FC))
```
...
x2DF <- scale(x2DF)
fit <- cv.glmnet(x = as.matrix(xDF), 
                 y = yDF$tpm_FC[match(rownames(xDF), table = yDF$subject_id)],
                 nfolds = nrow(xDF),
                 grouped = FALSE)
plot(fit)
coef(fit, s = fit$lambda.min) %>%
...
plot(yhat[, "s1"], y2DF$tpm_FC[match(rownames(x2DF), table = y2DF$subject_id)])
cor.test(yhat[, "s1"], y2DF$tpm_FC[match(rownames(x2DF), table = y2DF$subject_id)])

plot(rank(-1 * yhat[, "s1"]), rank(-1 * y2DF$tpm_FC[match(rownames(x2DF), table = y2DF$subject_id)]))
cor.test(rank(-1 * yhat[, "s1"]), rank(-1 * y2DF$tpm_FC[match(rownames(x2DF), table = y2DF$subject_id)]))
```
...
plot(yhat[as.character(y2DF$subject_id), "s1"], y2DF$tpm_FC)
cor.test(yhat[as.character(y2DF$subject_id), "s1"], y2DF$tpm_FC)

plot(rank(-1 * yhat[as.character(y2DF$subject_id), "s1"]), rank(-1 * y2DF$tpm_FC))
cor.test(rank(-1 * yhat[as.character(y2DF$subject_id), "s1"]), rank(-1 * y2DF$tpm_FC))
```
...
trainDF <- xDF %>%
  as.data.frame() %>%
  mutate(tpm_FC = yDF$tpm_FC[match(rownames(xDF), table = yDF$subject_id)])
...
plot(predict(auto_fit, new_data = as.data.frame(xDF))$.pred, 
     yDF$tpm_FC[match(rownames(xDF), table = yDF$subject_id)])


plot(predict(auto_fit, new_data = as.data.frame(x2DF))$.pred, 
     y2DF$tpm_FC[match(rownames(x2DF), table = y2DF$subject_id)])
...
cor.test(rank(-1 * predict(auto_fit, new_data = as.data.frame(x2DF))$.pred), 
     rank(-1 * y2DF$tpm_FC[match(rownames(x2DF), table = y2DF$subject_id)]))
```
...
trainDF <- rbind(xDF, x2DF) %>%
  as.data.frame() %>%
  mutate(tpm_FC = c(yDF$tpm_FC, y2DF$tpm_FC))
