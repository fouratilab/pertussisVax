...
yDF <- ab2020DF %>% 
  ...
  select(subject_id, planned_day_relative_to_boost, MFI_normalised) %>%
...
plot(yhat[, "s1"], rank(-1 * yobs$MFI_FC))
cor.test(yhat[, "s1"], rank(-1 * yobs$MFI_FC))
```
...
x2DF <- scale(x2DF)
fit <- cv.glmnet(x = as.matrix(xDF), 
                 y = rank(-yDF[match(rownames(xDF), table = yDF$subject_id), "MFI_FC"]),
                 nfolds = nrow(xDF),
                 grouped = FALSE)
plot(fit)
coef(fit, s = fit$lambda.min) %>%
...
plot(yhat[as.character(yobs$subject_id), "s1"], rank(-1 * yobs$MFI_FC))
cor.test(yhat[as.character(yobs$subject_id), "s1"], rank(-1 * yobs$MFI_FC))
```
...
trainDF <- rbind(xDF, x2DF) %>%
  as.data.frame() %>%
  mutate(MFI_FC = c(yDF$MFI_FC, yobs$MFI_FC))
